 ...
 * Time   : 2022/1/20 14:08
 * Desc   : 万能分割线
 */
public class DividerItemDecoration_Ma extends RecyclerView.ItemDecoration {
     ...
     */
    public DividerItemDecoration_Ma(Context context, int orientation) {
     ...
     */
    public DividerItemDecoration_Ma(Context context, int orientation, int drawableId) {
        this.setOrientation(orientation);
     ...
     * @param right         右边间距
     */
    public DividerItemDecoration_Ma(int dividerHeight, int left, int right, Context context) {
        MARGINRIGHT = dip2px(context, right);
        MARGINLEFT = dip2px(context, left);
        mOrientation = HORIZONTAL_LIST;

        mDividerHeight = dividerHeight;
        //抗锯齿画笔
        mPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        mPaint.setColor(Color.parseColor("#dfe3ea"));
        //填满颜色
        mPaint.setStyle(Paint.Style.FILL);
    }
    ...
    private void drawHorizontal(Canvas canvas, RecyclerView parent) {
        ...
        for (int i = 0; i < childSize; i++) {
            ...
            if (mPaint != null) {
                canvas.drawRect(x + MARGINLEFT, y, width - MARGINRIGHT, height, mPaint);
            }
        }
    }
